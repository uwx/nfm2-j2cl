...
public abstract class CubicCurve2D implements Shape, Cloneable {
	...
	public void setCurve(CubicCurve2D c) {
		setCurve(c.getX1(), c.getY1(), c.getCtrlX1(), c.getCtrlY1(), c.getCtrlX2(), c.getCtrlY2(), c.getX2(),
				c.getY2());
	}
	 ...
	 * @since 1.2
	 */
	public static double getFlatnessSq(double x1, double y1, double ctrlx1, double ctrly1, double ctrlx2, double ctrly2,
			double x2, double y2) {
	...
	public static int solveCubic(double eqn[], double res[]) {
		...
		if (D < 0) { /* Casus irreducibilis: three real solutions */
			...
			double err = 1200000000 * GWTMath.ulp(abs(uv) + abs(sub));
	...
	private static int fixRoots(double[] eqn, double[] res, int num) {
		...
		if (num == 3) {
			...
			// goodRoot = (abs(eqn'(res[0])) > abs(eqn'(res[1]))) ? res[0] :
			// res[1]
			...
			if (iszero(fx, 10000000 * GWTMath.ulp(x))) {
				double badRootVal = solveEqn(eqn, 3, badRoot);
				res[1] = abs(badRootVal) < abs(fx) ? badRoot : x;
				return 2;
			}
		} // else there can only be one root - goodRoot, and it is already in
			// res[0]
	...
	private static double refineRootWithHint(double[] eqn, double min, double max, double t) {
		...
		if (within(t, origt, 1000 * GWTMath.ulp(origt)) && inInterval(t, min, max)) {
			return t;
		}
		return origt;
	}
	...
	/*
	 * Computes M+1 where M is an upper bound for all the roots in of eqn. See:
	 * http://en.wikipedia.org/wiki/Sturm%27s_theorem#Applications. The above
	 * link doesn't contain a proof, but I [dlila] proved it myself so the
	 * result is reliable. The proof isn't difficult, but it's a bit long to
	 * include here. Precondition: eqn must represent a cubic polynomial
	 */
	...
	public boolean contains(double x, double y) {
		...
		double x2 = getX2();
		double y2 = getY2();
		int crossings = (Curve.pointCrossingsForLine(x, y, x1, y1, x2, y2) + Curve.pointCrossingsForCubic(x, y, x1, y1,
				getCtrlX1(), getCtrlY1(), getCtrlX2(), getCtrlY2(), x2, y2, 0));
	...
	private int rectCrossings(double x, double y, double w, double h) {
		...
		return Curve.rectCrossingsForCubic(crossings, x, y, x + w, y + h, getX2(), getY2(), getCtrlX2(), getCtrlY2(),
				getCtrlX1(), getCtrlY1(), getX1(), getY1(), 0);
	}
	 ...
	 */
	public Object clone() {
		return ShallowClone.clone(this);
	}
}
