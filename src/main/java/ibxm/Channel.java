...
public class Channel {
   ...
   public void row(Note note) {
      ...
      switch (this.noteEffect) {
         ...
         case 122:
            ...
            this.volume = this.volume + this.fineVslideUpParam;
            if (this.volume > 64) {
               this.volume = 64;
            }
            break;
         case 123:
            ...
            this.volume = this.volume - this.fineVslideDownParam;
            if (this.volume < 0) {
               this.volume = 0;
            }
            break;
   ...
   public void tick() {
      ...
      if (this.noteEffect != 125 || this.fxCount > this.noteParam) {
         switch (this.noteVol & 240) {
            case 96:
               this.volume = this.volume - (this.noteVol & 15);
               if (this.volume < 0) {
                  this.volume = 0;
               }
               break;
            case 112:
               ...
               break;
            case 176:
               this.vibratoPhase = this.vibratoPhase + this.vibratoSpeed;
               this.vibrato(false);
               break;
            case 208:
               this.panning = this.panning - (this.noteVol & 15);
               if (this.panning < 0) {
                  this.panning = 0;
               }
               break;
      ...
      switch (this.noteEffect) {
         ...
         case 7:
         case 146:
            this.tremoloPhase = this.tremoloPhase + this.tremoloSpeed;
            this.tremolo();
            break;
         case 10:
         case 132:
            ...
            break;
         case 17:
            this.globalVol.volume = this.globalVol.volume + ((this.globalVslideParam >> 4) - (this.globalVslideParam & 15));
            ...
            break;
         case 25:
            this.panning = this.panning + ((this.panningSlideParam >> 4) - (this.panningSlideParam & 15));
            if (this.panning < 0) {
               this.panning = 0;
            }
   ...
   private void updateEnvelopes() {
      if (this.instrument.volumeEnvelope.enabled) {
         if (!this.keyOn) {
            this.fadeOutVol = this.fadeOutVol - this.instrument.volumeFadeOut;
   ...
   private void autoVibrato() {
      ...
      if (depth > 0) {
         ...
         this.vibratoAdd = this.vibratoAdd + (this.waveform(this.autoVibratoCount * rate >> 2, type + 4) * depth >> 8);
   ...
   private void tonePortamento() {
      if (this.period > 0) {
         if (this.period < this.portaPeriod) {
            this.period = this.period + (this.tonePortaParam << 2);
            if (this.period > this.portaPeriod) {
   ...
   private int waveform(int phase, int type) {
      ...
      switch (type) {
         case 1:
         case 7:
            amplitude = 255 - ((phase + 32 & 63) << 3);
            break;
         case 2:
         case 5:
            amplitude = (phase & 32) > 0 ? 255 : -255;
            break;
         case 3:
         case 8:
         ...
         case 4:
         default:
            amplitude = sineTable[phase & 31];
            if ((phase & 32) > 0) {
               amplitude = -amplitude;
            }
            break;
         case 6:
            amplitude = ((phase + 32 & 63) << 3) - 255;
      }
      ...
      if (this.retrigCount >= this.retrigTicks) {
         ...
         switch (this.retrigVolume) {
            ...
            case 5:
               ...
               break;
            case 6:
               this.volume = this.volume - this.volume / 3;
               break;
            case 7:
               this.volume >>= 1;
            case 8:
   ...
   private void trigger() {
      if (this.noteIns > 0 && this.noteIns <= this.module.numInstruments) {
         ...
         this.volume = sam.volume >= 64 ? 64 : sam.volume & 63;
         if (sam.panning >= 0) {
      ...
      if (this.noteKey > 0) {
         if (this.noteKey > 96) {
            ...
            boolean isPorta = (this.noteVol & 240) == 240 || this.noteEffect == 3 || this.noteEffect == 5 || this.noteEffect == 135 || this.noteEffect == 140;
            if (!isPorta) {
