...
public class ContO {
    ...
    public ContO(String[] lines, Trackers trackers) {
        t = trackers;
        p = new Plane[270];
        boolean flag = false;
        boolean flag1 = false;
        ...
        if (Utility.cXs(i + maxR * 2, k) > 0 && Utility.cXs(i - maxR * 2, k) < Medium.w && k > -maxR && (k < Medium.fade[disline] + maxR || Medium.trk)
            ...
            if (Utility.cYs(j1 + maxR, k) > 0 && Utility.cYs(j1 - maxR, k) < Medium.h) {
                ...
                for (int l3 = 0; l3 < npl; l3++) {
                    p[l3].d(rd, x - Medium.x, y - Medium.y, z - Medium.z, xz, xy, zy, wxz, wzy, noline, l);
                    if (p[l3].master != 0 && stg[p[l3].master - 1] != 0) {
                        pdust(p[l3].master - 1, rd, 1);
                    }
                }
