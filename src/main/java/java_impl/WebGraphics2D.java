...
public class WebGraphics2D extends Graphics2D {
	...
	public WebGraphics2D(HTMLCanvasElement canvas) {
		this.context = Js.uncheckedCast(canvas.getContext("2d"));
	}
	...
	public void clearRect(int x, int y, int width, int height) {
		FillStyleUnionType oldFillStyle = context.fillStyle;
		context.fillStyle = FillStyleUnionType.of(backgroundHtml);
...
//				context.fill();
	...
	public static BaseRenderingContext2D.DrawImageImageUnionType getDrawableImage(Image img) {
		if (img == null) {
			throw new RuntimeException("Image is null!!");
		}
		BaseRenderingContext2D.DrawImageImageUnionType drawableImage = ((Drawable) img).getDrawableImage();
	...
	private Color color;
	private static final JsMap<JsNumber, String> colorToString = new JsMap<>();

	private static String getColorHtml(Color c) {
		JsNumber rgb = new JsNumber(c.getRGB());
		String html = colorToString.get(rgb);
        if (html != null) {
            return html;
        }

        html = c.toHTML();
        colorToString.set(rgb, html);
        return html;
    }
	...
	public void setColor(Color c) {
		color = c;
		String html = getColorHtml(c);
		context.strokeStyle = StrokeStyleUnionType.of(html);
		context.fillStyle = FillStyleUnionType.of(html);
	}
	...
	public void setPaint(Paint paint) {
		if ( paint instanceof Color )
			context.fillStyle = FillStyleUnionType.of(((Color) paint).toHTML());
	}
