...
public class BorderLayout implements LayoutManager2, java.io.Serializable {
	 ...
	 *            to the layout.
	 * @see Container#add(Component, java.lang.Object)
	...
	public void removeLayoutComponent(Component comp) {
		((HTMLComponent)comp).getHTMLElement().parentNode.removeChild(((HTMLComponent)comp).getHTMLElement());
	...
	public void layoutContainer(Container parent) {
		if (!created) {
			this.parent = parent;
			created = true;
			elemental2.dom.HTMLDivElement div = (HTMLDivElement) ((HTMLComponent)parent).getHTMLElement();
			table = (HTMLTableElement) document.createElement("table");
			table.style.width = WidthUnionType.of("100%");
			table.style.height = HeightUnionType.of("100%");
			// table.style.position = "absolute";
			table.style.left = "0px";
			table.style.right = "0px";
			table.style.zIndex = ZIndexUnionType.of("0");
			...
			for (int j = 0; j < 3; j++) {
				...
				if(j != 1) {
					row.style.height = HeightUnionType.of("0px");
				} else {
					row.style.height = HeightUnionType.of("100%");
				}
				for (int i = 0; i < 3; i++) {
					if (j == 1 || i == 1) {
						HTMLTableCellElement col = (HTMLTableCellElement) document.createElement("td");
						col.style.padding = PaddingUnionType.of("0px");
						row.appendChild(col);
						if (i == 1) {
							col.style.width = WidthUnionType.of("100%");
							if (j != 1) {
								col.colSpan = 3;
							}
						} else {
							col.style.width = WidthUnionType.of("0px");
						}
					}
				}
			}
		...
		for (int j = 0; j < 3; j++) {
			for (int i = 0; i < 3; i++) {
				if (pos++ == position) {
					...
					HTMLTableColElement col = (HTMLTableColElement) row.childNodes.getAt(j == 1 ? i : 0);
					col.appendChild(((HTMLComponent)component).getHTMLElement());
					return;
				}
			}
		}
