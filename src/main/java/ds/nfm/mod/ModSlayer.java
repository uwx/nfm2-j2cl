...
public class ModSlayer extends ModuleSlayer {
   ...
   final int get_track(ModTrackInfo modtrackinfo, byte[] is, int i) {
      ...
      modtrackinfo.noterestart = modtrackinfo.noterestart - this.tempo;
      ...
      if (i_4_ != 0 || i_5_ != 0) {
         switch (i_4_) {
            case 0:
               ...
               while (i_6_ < 48 && modtrackinfo.period < period_set[i_6_]) {
                  i_6_++;
               }

               modtrackinfo.arp[0] = period_set[i_6_];
               modtrackinfo.arp[1] = period_set[i_6_ + (i_5_ & 15)];
               modtrackinfo.arp[2] = period_set[i_6_ + ((i_5_ & 240) >> 4)];
               modtrackinfo.arpindex = 0;
      ...
      if (modtrackinfo.replen < 3) {
         ...
         if (samplepos < endtr) {
            ...
            if (modtrackinfo.pitch < 4096) {
               while (samplepos < endtr && buffpos < buffend) {
                  ...
                  int var14;
                  samplepos += hipitch + ((var14 = error + lopitch) >> 12);
                  error = var14 & 4095;
               }
            } else {
               while (samplepos < endtr && buffpos < buffend) {
                  ...
                  int var13;
                  samplepos += hipitch + ((var13 = error + lopitch) >> 12);
                  error = var13 & 4095;
               }
            }
         ...
         if (modtrackinfo.pitch < 4096) {
            while (bufflen > 0) {
               ...
               int var16;
               samplepos += hipitch + ((var16 = error + lopitch) >> 12);
               error = var16 & 4095;
               bufflen--;
            }
         } else {
            while (bufflen > 0) {
               ...
               int var15;
               samplepos += hipitch + ((var15 = error + lopitch) >> 12);
               error = var15 & 4095;
               bufflen--;
            }
         }
   ...
   public byte[] turnbytesNorm(boolean calvol) throws IOException {
      ...
      byte[] realbytes = new byte[18000000];
         ...
         if (!this.mod_done) {
            ...
            if (this.oversample > 1) {
               ...
               if (this.oversample == 2) {
                  for (int var12 = 0; var12 < real_samples; var12++) {
                     buf[var12] = buf[k] + buf[k + 1] >> 1;
                     k += 2;
                  }
               } else {
                  for (int var13 = 0; var13 < real_samples; var13++) {
                     int sum = buf[k++];
            ...
            if (this.oln + real_samples < 18000000) {
               if (calvol) {
                  ...
                  for (int var14 = 0; var14 < real_samples; var14++) {
                     if (buf[var14] > 0) {
                        cav += buf[var14];
                        niu++;
                     }
